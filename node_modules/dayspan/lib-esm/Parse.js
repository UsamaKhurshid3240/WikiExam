...
export class Parse {
     ...
     */
    static modifier(input, value, parseMeta = (x => x), out = new ScheduleModifier()) {
        let map = {};
        if (fn.isArray(input)) {
     ...
     */
    static schedule(input, parseMeta = (x => x), out = new Schedule()) {
     ...
     */
    static cron(pattern, out = new Schedule()) {
        return out;
    }
}
